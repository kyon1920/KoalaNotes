...
MuItemDelegate::MuItemDelegate(int x, QObject *parent) :
    QStyledItemDelegate(parent), x(x)
{
...
void MuItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.isValid()) {
        ...
        // 绘制图片，地址，数量位置区域
        QRectF iconRect = QRect(rect.left()+5, rect.top()+5, x, 40);
        ...
        painter->drawText(songNbRect, itemData.dates);
